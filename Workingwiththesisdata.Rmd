...
ipos_yearly <- read.csv("annual_ipos_2000_2025.csv")
# rename column to year
ipos_yearly <- ipos_yearly %>%
  rename(year = Year) %>%
  filter(year >= 2000, year <= 2022) %>% select(year, IPOs)
...
#plot fed rates over time
ggplot(exogenous, aes(x = year, y = fed_rate)) +
  geom_line(size = 1, color = "blue") +
  labs(title = "Federal Reserve Rates Over Time", x = "Year", y = "Fed Rate (%)") +
  theme_minimal()
...
div_panel_model <- div_panel_model %>%
  ...
  mutate(
    ...
    lag6_win_med   = compute_lagged_perf(period_2yr, lp, gap = 6, window_length = 10)[["median"]],
    lag6_win_mean  = compute_lagged_perf(period_2yr, lp, gap = 6, window_length = 10)[["mean"]],
    lag6_nowin_med = compute_lagged_perf(period_2yr, lp, gap = 6, window_length = NA)[["median"]],
    lag6_nowin_mean= compute_lagged_perf(period_2yr, lp, gap = 6, window_length = NA)[["mean"]],
...
summary(div_panel_model_filtered$lag4_nowin_med)

panel_model_normalized <- plm(model_formula, 
                   data = div_panel_model_filtered, 
                   index = c("lp", "period_2yr"),
                   model = "within", within = TRUE, effect = "individual")
summary(panel_model_normalized)
model_formula_raw <- divergence_prop ~ fed_rate + trail_1yr + num_funds + avg_size + avg_spread + lag4_nowin_med + AUM_line + LPType + log_cum_total_funds + log_current_lag_avg_close_size + log_current_lag_avg_manager_aum + period_factor
panel_model_raw <- plm(model_formula_raw, 
                   data = div_panel_model_filtered, 
                   index = c("lp", "period_2yr"),
                   model = "within", within = TRUE, effect = "individual")
summary(panel_model_raw)



model_formula_for_vif <- model_formula <- normalized_div_prop ~ fed_rate + trail_1yr + num_funds + avg_size + avg_spread + lag4_nowin_med + AUM_line + log_cum_total_funds + log_current_lag_avg_close_size + log_current_lag_avg_manager_aum + IPOs
lm_model_for_vif <- lm(model_formula_for_vif, data = div_panel_model_filtered)
vif_values <- vif(lm_model_for_vif)
print(vif_values)

# (a) Breusch-Godfrey test for serial correlation in panel data.
bg_test <- pbgtest(panel_model_normalized)
print(bg_test)

# (b) Pesaran's test for cross-sectional dependence.
csd_test <- pcdtest(panel_model_normalized, test = "cd")
print(csd_test)

# (c) Breusch-Pagan test for heteroskedasticity.
bp_test <- bptest(panel_model_normalized)
print(bp_test)


bg_test <- pbgtest(panel_model_raw)
print(bg_test)

# (b) Pesaran's test for cross-sectional dependence.
csd_test <- pcdtest(panel_model_raw, test = "cd")
print(csd_test)

# (c) Breusch-Pagan test for heteroskedasticity.
bp_test <- bptest(panel_model_raw)
print(bp_test)

panel_regression_cov <- vcovHC(panel_model_normalized, type = "HC1", cluster = "group", method = "arellano")
coeftest(panel_model_normalized, vcov = panel_regression_cov)

div_panel_model_filtered_short <- div_panel_model_filtered %>% filter(period_2yr >=2010)
panel_model_normalized_short <- panel_model_normalized <- plm(model_formula, 
                   data = div_panel_model_filtered_short, 
                   index = c("lp", "period_2yr"),
                   model = "within", within = TRUE, effect = "individual")
summary(panel_model_normalized_short)
bg_test <- pbgtest(panel_model_normalized_short)
print(bg_test)

# (b) Pesaran's test for cross-sectional dependence.
csd_test <- pcdtest(panel_model_normalized_short, test = "cd")
print(csd_test)

# (c) Breusch-Pagan test for heteroskedasticity.
bp_test <- bptest(panel_model_normalized_short)
print(bp_test)

panel_regression_short_cov <- vcovHC(panel_model_normalized, type = "HC1", cluster = "group", method = "arellano")
coeftest(panel_model_normalized_short, vcov = panel_regression_short_cov)
...
model_formula <- normalized_div_prop ~ fed_rate + trail_1yr + num_funds + avg_size + avg_spread + lag4_nowin_med + AUM_line + LPType + log_current_lag_avg_close_size + log_current_lag_avg_manager_aum
...
performance_metrics <- c("lag2_win_med", "lag2_win_mean",
                    ...
                    "lag4_excess_med", "lag6_win_med", "lag6_win_mean",
                    "lag6_nowin_med", "lag6_nowin_mean")
...
hausman_result <- phtest(panel_model,
                         panel_model_re)
