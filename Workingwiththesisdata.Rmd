...
#────────────────────────────────────────────
# STEP 1: Create a fund-level performance dataset.
# (Assuming final_modeling_filtered has the relevant columns)
fund_perf <- final_modeling_filtered %>%
  select(lp, vintage_inception_year, net_irr)  # you can add dpi, rvpi as needed

#────────────────────────────────────────────
# STEP 2: Create a function to compute lagged performance.
# For a given LP and current period (year), and given a trailing gap and (optionally) a window,
# return a named vector with median and mean net_irr.
compute_lagged_perf <- function(current_year, lp_id, gap, window_length = NA) {
  # For "windowed" version, if window_length is provided (e.g. 10), then use funds with vintage in:
  # [current_year - window_length, current_year - gap]
  # For "no window" version, use all funds with vintage <= current_year - gap.
  if (!is.na(window_length)) {
    lower_bound <- current_year - window_length
  } else {
    lower_bound <- -Inf  # take all available
  }
  upper_bound <- current_year - gap
  
  # Filter funds for this LP with vintage in the appropriate interval.
  vals <- fund_perf %>%
    filter(lp == lp_id,
...
#────────────────────────────────────────────
# STEP 3: Integrate lagged performance into the panel data.
# We want to compute lagged performance for various trailing gaps (2,3,4 years) 
# and with/without a 10-year window.
# We'll do this rowwise in the panel dataset.

# Assume div_panel_model has columns: lp, period_2yr, divergence_prop, normalized_div_prop, fed_rate, annual_return, etc.
div_panel_model <- div_panel_model %>%
  rowwise() %>%
  mutate(
    # For a 2-year gap with a 10-year window:
    lag2_win = list(compute_lagged_perf(period_2yr, lp, gap = 2, window_length = 10)),
    # For a 2-year gap, no window (i.e. all funds before period-2):
    lag2_nowin = list(compute_lagged_perf(period_2yr, lp, gap = 2, window_length = NA)),
    # For a 3-year gap with a 10-year window:
    lag3_win = list(compute_lagged_perf(period_2yr, lp, gap = 3, window_length = 10)),
    # For a 3-year gap, no window:
    lag3_nowin = list(compute_lagged_perf(period_2yr, lp, gap = 3, window_length = NA)),
    # For a 4-year gap with a 10-year window:
    lag4_win = list(compute_lagged_perf(period_2yr, lp, gap = 4, window_length = 10)),
    # For a 4-year gap, no window:
    lag4_nowin = list(compute_lagged_perf(period_2yr, lp, gap = 4, window_length = NA))
  ) %>%
  # Now extract the median and mean components into separate columns.
  mutate(
    lag2_win_med = lag2_win[[1]]["median"],
    lag2_win_mean = lag2_win[[1]]["mean"],
    lag2_nowin_med = lag2_nowin[[1]]["median"],
    lag2_nowin_mean = lag2_nowin[[1]]["mean"],
    lag3_win_med = lag3_win[[1]]["median"],
    lag3_win_mean = lag3_win[[1]]["mean"],
    lag3_nowin_med = lag3_nowin[[1]]["median"],
    lag3_nowin_mean = lag3_nowin[[1]]["mean"],
    lag4_win_med = lag4_win[[1]]["median"],
    lag4_win_mean = lag4_win[[1]]["mean"],
    lag4_nowin_med = lag4_nowin[[1]]["median"],
    lag4_nowin_mean = lag4_nowin[[1]]["mean"]
  ) %>%
  ungroup()

#────────────────────────────────────────────
# At this point, div_panel_model has additional columns (e.g., lag2_win_med, lag2_nowin_mean, etc.)
# that you can now use as predictors in your panel regression.
#
# For example, using the plm package (for a fixed effects model):
library(plm)
panel_model <- plm(annual_return ~ lag2_win_med + lag2_win_mean +
                     ...
                     fed_rate,  # and other variables as needed
                   data = div_panel_model,
